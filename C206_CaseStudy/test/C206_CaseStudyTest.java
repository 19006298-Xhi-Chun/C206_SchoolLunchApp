...
public class C206_CaseStudyTest {
	...
	public void setUp() throws Exception {
		...
		order1 = new Order("1900000", "12/01/12", menuItemList.toString());
	...
	public void updateOrderDateTest() {
		...
		assertNotNull("Check the order list is not null", orderList);

		// Given that list is not empty check that student id is the same as recorded-
		// Normal
		boolean same = order1.getStudentId().equalsIgnoreCase("1900000");
		assertTrue("Check if the student is the same as recorded", same);

		// check if the date can be updated - normal
		String newDate = "23-07-2004";
		order1.setOrderDate(newDate);
		String expected = String.format("%-30s %-30s %-30s", "1900000", "23-07-2004", menuItemList.toString());
		String output = String.format("%-30s %-30s %-30s", order1.getStudentId(), order1.getOrderDate(),
				menuItemList.toString());

		assertEquals("Check if the new information vcan be updated", output, expected);

		// List was not empty and enter in the change orderDate but it was not updated
		// due to student id mismatch - error
		boolean different = order1.getStudentId().equalsIgnoreCase("190000002");
		assertFalse("Check the student id is different as recorded", different);
		newDate = "23-07-2004";
		order1.setOrderDate(newDate);
		expected = String.format("%-30s %-30s %-30s", "1900002", "23-07-2004", menuItemList.toString());
		output = String.format("%-30s %-30s %-30s", order1.getStudentId(), order1.getOrderDate(),
				menuItemList.toString());

		assertNotEquals("Check if the new information vcan be updated", output, expected);
	...
	public void SearchStudentIdTest() {
		...
		assertFalse("Ensure that the order list si not empty",notEmpty);
	...
	public void AddOrderTest() {
		...
		// amount -error
		menuItemList.clear();
		boolean isEmpty = menuItemList.isEmpty();
		assertTrue("Verfy that item menu list is emtpy", isEmpty);
		double expected = 0;
		double output = 12;
		if (isEmpty == true) {
			output = 0;
		}
		boolean sameAmount = output == expected;

		assertTrue("verify that total price and the expected price == 0", sameAmount);
